...
export default function Contact() {
  ...
  const SUBJECTS = [
    { label: t("contact.subjects.commissionGroup"), disabled: true },
    { label: t("contact.subjects.customMlo"), value: "Custom MLO" },
    { label: t("contact.subjects.exteriorMapping"), value: "Exterior Mapping" },
    { label: t("contact.subjects.optimization"), value: "Optimization" },
    { label: t("contact.subjects.supportGroup"), disabled: true },
    { label: t("contact.subjects.bugReport"), value: "Bug Report" },
    { label: t("contact.subjects.complaint"), value: "Complaint" },
    { label: t("contact.subjects.generalQuestion"), value: "General Question" },
  ...
  const BUDGETS = [
    { value: "< 20€",     label: t("contact.budgets.under20") },
    { value: "20–50€",    label: t("contact.budgets.20to50") },
    { value: "50–100€",   label: t("contact.budgets.50to100") },
    { value: "100–200€",  label: t("contact.budgets.100to200") },
    { value: "> 200€",    label: t("contact.budgets.over200") },
    { value: "À discuter", label: t("contact.budgets.discuss") },
  ];

  const TIMELINES = [
    { value: "< 1 semaine",  label: t("contact.timelines.under1week") },
    { value: "1–2 semaines", label: t("contact.timelines.1to2weeks") },
    { value: "2–4 semaines", label: t("contact.timelines.2to4weeks") },
    { value: "> 1 mois",     label: t("contact.timelines.over1month") },
  ...
  function validate() {
    const e = {};
    if (!form.email || !EMAIL_RE.test(form.email)) e.email = t("contact.errors.emailRequired");
    if (!form.subject) e.subject = t("contact.errors.subjectRequired");
  ...
  if (status === "success") {
    return (
      <div className="mx-auto max-w-2xl px-6 py-24 text-center">
        ...
        <h2 className="text-3xl font-extrabold text-white/90 mb-3">{t("contact.successTitle")}</h2>
        {ticketNumber && (
          <p className="text-[#a89fff] font-mono text-lg font-bold mb-3">{ticketNumber}</p>
        )}
        <p className="text-white/55 mb-8">{t("contact.successText")}</p>

        {buildDiscordOAuthUrl(ticketNumber) && (
          <div className="mb-6">
            <p className="text-white/45 text-sm mb-3">{t("contact.discordJoinText")}</p>
            <a
  ...
  return (
    <div className="mx-auto max-w-2xl px-6 pb-16 pt-8">
      ...
      <form onSubmit={handleSubmit} noValidate className="mt-10 flex flex-col gap-5">
        ...
        <Field label={t("contact.discordLabel")} optional={t("contact.optional")}>
          <input
            type="text"
            name="discord"
            value={form.discord}
        ...
        <Field label={t("contact.subjectLabel")}>
          ...
          >
            <option value="" disabled className="bg-[#0f1117]">{t("contact.subjectPlaceholder")}</option>
            {SUBJECTS.map((s) =>
              s.disabled ? (
        ...
        {COMMISSION_SUBJECTS.includes(form.subject) && (
          <div className="grid grid-cols-2 gap-4">
            <Field label={t("contact.budgetLabel")} optional={t("contact.optional")}>
              <select
                name="budget"
                value={form.budget}
                onChange={handleChange}
                className={inputClass + " appearance-none cursor-pointer"}
              >
                <option value="" className="bg-[#0f1117]">{t("contact.notSpecified")}</option>
            ...
            <Field label={t("contact.timelineLabel")} optional={t("contact.optional")}>
              <select
                name="timeline"
                value={form.timeline}
                onChange={handleChange}
